...
@click.option('--no-backup', is_flag=True, default=False,
              help='Do not generate backup of original archive file(s).')
def batchreplace(encoding, lsb_file, script_dir, no_backup):
    """Compile specified LNS script directory and replace existing Scenarios in the LSB file.

    script_dir should be an LNS script directory which was initially generated by lmlsb extract, because an equal file naming is required.
    Different to the batchimport function it does not insert new script files, but is way faster.
    ...
    for index, name, scenario in lsb.text_scenarios():
        if name:
            script_name = '{}-{}.lns'.format(lsb_path.stem, name)
        else:
            script_name = '{}-line{}.lns'.format(lsb_path.stem, index)
        script_file = script_dir.joinpath(script_name)
        if Path(script_file).exists():
            ...
            try:
                ...
                new_body = cc.compile(script)
                scenario.replace_body(new_body)
                print('Script at line {} replaced.'.format(index))
            except LiveMakerException as e:
                print('Could not compile script file: {}, Script at line {} remain original.'.format(script_name, index), file=sys.stderr)
                print('Compilation error: {}, '.format(e), file=sys.stderr)

        else:
            print('File {} not found, Script at line {} remain original.'.format(script_name, index), file=sys.stderr)
