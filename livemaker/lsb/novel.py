...
class TpWord(BaseSerializable):
    ...
    def to_xml(self):
        ...
        if "\x01" in xml:
            logger.warn("Removing illegal xml char \\x01")
            xml = xml.replace("\x01", "*")
...
class LNSCompiler(_markupbase.ParserBase):
    ...
    def compile(self, script):
        ...
        if not script.startswith(";pylm"):
            logger.warn("Attempting to compile script which was not generated by pylivemaker.")
            ...
            if startswith("<", i) or startswith("{", i):
                if starttagopen.match(rawdata, i):  # < + letter
                    ...
                    if startswith("{}"):
                        ...
                        logger.warn("got < or { at end of line")
                        self.handle_data("<")
                        k = i + 1
                else:
                    break
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == LNSTag.a:
            ...
            if self.version and self.version < 102:
                ...
                # a live parser
                logger.warn("Compiling script ver < 102 with VAR tags not fully supported")
            d["var_name"] = attrs.get("NAME", "")
            d["unk3"] = int(attrs.get("unk3", 0))
